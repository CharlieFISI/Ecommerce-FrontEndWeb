...
export const ProductListItem = ({ 
  ...
  const totalStock = product.listings.reduce((sum, listing) => sum + listing.stock, 0);
  ...
  return (
    <Pressable
      className={`flex-row mb-4 bg-gray-900 rounded-lg overflow-hidden ${isOutOfStock ? 'opacity-50' : ''}`}
      onPress={onPress}
    >
      <Image
        source={
      ...
      >
        <AntDesign 
          name={showAddToCart ? "shoppingcart" : (product.isFavorite ? "heart" : "hearto")} 
          size={24} 
