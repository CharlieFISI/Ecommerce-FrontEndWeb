    ...
    <ScrollView className="flex-1 bg-[#121212]">
      ...
      <View className="relative h-[400px]">
        ...
        <View className="absolute bottom-8 left-4">
          <Text className="mb-4 text-4xl font-bold text-white">Electronics sale</Text>
        </View>
      </View>
      ...
      {topCategories?.map((category) => (
        <CategoryProductSection key={category.id} category={category} router={router} />
      ))}
...
const CategoryProductSection: React.FC<{ category: Category; router: Router }> = ({ category, router }) => {
  const { data: categoryProducts, isLoading } = useProductsByCategory(category.id);
