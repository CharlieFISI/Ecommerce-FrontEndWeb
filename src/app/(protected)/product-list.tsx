...
import { useProducts, useProductsByCategory, ProductInfo } from "../../hooks/useProducts";
...
const ProductListScreen = () => {
  ...
  const { data: categoryProducts, isLoading: categoryProductsLoading } = useProductsByCategory(categoryId as string);
    ...
    <ScrollView className="flex-1 bg-[#121212]">
      ...
      >
        <Pressable
          ...
          className={`px-4 py-2 rounded-full mr-2 ${!categoryId ? 'bg-[#4A90E2]' : 'bg-gray-800'}`}
        >
        ...
        {categories?.map((category) => (
          <Pressable
            ...
            className={`px-4 py-2 rounded-full mr-2 ${category.id === categoryId ? 'bg-[#4A90E2]' : 'bg-gray-800'}`}
          >
      ...
      <View className="flex-row items-center justify-between px-4 mb-4">
        ...
        <Pressable onPress={() => setViewMode(viewMode === 'grid' ? 'list' : 'grid')}>
          <AntDesign name={viewMode === 'grid' ? 'bars' : 'appstore-o'} size={20} color="white" />
        </Pressable>
      </View>
      ...
      <View className="px-4">
        ...
        {filteredAndSortedProducts.length > 0 ? (
          viewMode === 'grid' ? (
            <View className="flex-row flex-wrap justify-between mt-4">
              {filteredAndSortedProducts.map((product) => (
                <View key={product.id} style={{ width: (SCREEN_WIDTH - 48) / 2 }} className="mb-4">
                  <ProductGridItem
                    product={product}
                    onPress={() => router.push({
                      pathname: '/product',
          ...
          <Text className="mt-4 text-center text-white">No products found matching the current filters.</Text>
        )}
      </View>
      ...
      >
        <View className="justify-end flex-1 bg-black bg-opacity-50">
          <View className="bg-[#121212] p-4 rounded-t-3xl">
            <Text className="mb-4 text-xl font-bold text-white">Sort by</Text>
            <Pressable
              className="py-3"
              onPress={() => {
                setSortBy('newest');
            ...
            >
              <Text className={`text-white text-lg ${sortBy === 'newest' ? 'font-bold' : ''}`}>Newest</Text>
            </Pressable>
            <Pressable
              className="py-3"
              onPress={() => {
            ...
            >
              <Text className={`text-white text-lg ${sortBy === 'price-asc' ? 'font-bold' : ''}`}>Price: Low to High</Text>
            </Pressable>
            <Pressable
              className="py-3"
              onPress={() => {
            ...
            >
              <Text className={`text-white text-lg ${sortBy === 'price-desc' ? 'font-bold' : ''}`}>Price: High to Low</Text>
            </Pressable>
          </View>
        </View>
      </Modal>
      ...
      >
        <View className="justify-end flex-1 bg-black bg-opacity-50">
          <View className="bg-[#121212] p-4 rounded-t-3xl">
            <Text className="mb-4 text-xl font-bold text-white">Price Range</Text>
            
            <MultiSlider
              values={tempPriceRange}
              sliderLength={SCREEN_WIDTH - 60}
              onValuesChange={(values) => setTempPriceRange(values as [number, number])}
              min={0}
              max={maxPrice}
              step={1}
              allowOverlap={false}
