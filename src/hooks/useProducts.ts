...
const fetchProductListings = async (productId: string): Promise<Listing[]> => {
  ...
  return listings.map(({ stock, price, sellerName, sellerId }: any) => ({ stock, price, sellerName, sellerId }));
};
...
export const useCart = () => {
  ...
  const fetchCartItems = async (): Promise<CartItemWithDetails[]> => {
    ...
    return Promise.all(cartItems.map(async (item) => {
      const product = products.find(p => p.id === item.productListing.productId);
      if (!product) {
        throw new Error(`Product not found for id: ${item.productListing.productId}`);
      }
      const listings = await fetchProductListings(product.id);
      const listing = listings.find(l => l.price === item.productListing.price);
      if (!listing) {
  ...
  const updateCartItemQuantity = async ({ productId, sellerId, quantity }: { productId: string; sellerId: string; quantity: number }) => {
    const token = await getToken();
    const response = await fetch(`${ApiURL}/api/cart`, {
      method: 'PUT',
      headers: {
  ...
  const removeCartItem = async ({ productId, sellerId }: { productId: string; sellerId: string }) => {
    const token = await getToken();
    const response = await fetch(`${ApiURL}/api/cart`, {
      method: 'DELETE',
      headers: {
  ...
  const addToCart = async ({ productId, sellerId, quantity }: { productId: string; sellerId: string; quantity: number }) => {
    const token = await getToken();
    const response = await fetch(`${ApiURL}/api/cart`, {
      method: 'POST',
      headers: {
  ...
  const { data: cartItems, isLoading, error } = useQuery<CartItemWithDetails[], Error>({
    queryKey: ['cart'],
  ...
  const calculateTotal = () => {
    ...
    return cartItems.reduce((total, item) => total + item.quantity * item.productListing.price, 0);
  };
